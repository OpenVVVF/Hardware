...
The tool reads pasted SendCutSend cart text (from a file or stdin), extracts
part details, and writes info.txt files into the matching folders under
Hardware/Chassis*/Mechanical/Fab/.

Example usage:
    python3 import_sendcutsend_cart.py < cart.txt
        ...
        with open(info_path, "r", encoding="utf-8") as f:
            for line in f:
                if line.strip().startswith("Notes="):
                    existing_notes = line.strip().split("=", 1)[1]
                elif line.strip().startswith("PartName="):
                    existing_part_name = line.strip().split("=", 1)[1]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Import SendCutSend cart text into info.txt files.")
    parser.add_argument("input", nargs="?", help="Cart text file (default: stdin)")
    parser.add_argument("--chassis", default="Chassis2", help="Chassis folder name (default: Chassis2)")
    parser.add_argument("--hardware-root", type=Path, default=None, help="Hardware root directory")
    ...
    chassis_dir = hardware_root / args.chassis
