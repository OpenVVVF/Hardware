"""Import McMaster order/cart prices into the price cache and part database.
...
def run(argv, ctx: Context) -> int:
    parser = argparse.ArgumentParser(prog="import mcmaster", description="Import McMaster order/cart prices.")
    ...
    if imported:
        updated = persist_prices_to_db(ctx.db, "mcmaster", imported)
        if updated:
            ctx.db.save()
